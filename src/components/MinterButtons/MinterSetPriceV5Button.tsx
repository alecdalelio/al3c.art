...
const MinterSetPriceV5Button = ({
  ...
  let customRequest = config.request
    ? {
        ...
        gasLimit: multiplyBigNumberByFloat(
  ...
  const mintingDisabled =
    isPaused || isSoldOut || !isConnected || !verifyBalance;
  let mintingMessage = `${
    artistCanMint ? "Artist Mint " : "Purchase "
  } for ${formatEtherFixed(priceWei.toString(), 3)} ${currencySymbol}`;
  ...
  return (
    <>
      ...
      />
      <Box marginTop={1}>
        <Typography fontStyle="italic">{dialog}</Typography>
      </Box>
      <Modal
        open={mintingPreview}
